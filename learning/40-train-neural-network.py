...
class ChessEvalNet(nn.Module):
    ...
    def __init__(
        self, input_size: int = 779, hidden_sizes: list[int] = [512, 256, 128]
    ):
...
def train_neural_network(
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch + 1}: Train RMSE: {train_rmse:.2f}, Val RMSE: {val_rmse:.2f}, Val MAE: {val_mae:.2f}"
        )
        ...
        if plot_live:
            ...
            # Plot RMSE
            ax1.plot(
                range(1, epoch + 2), train_rmses, "b-", label="Train RMSE", linewidth=2
            )
            ax1.plot(
                range(1, epoch + 2), val_rmses, "r-", label="Val RMSE", linewidth=2
            )
            ...
            fig.suptitle(
                f"Neural Network Training Progress - {model_suffix} (LR: {current_lr:.2e})",
                fontsize=14,
            )
    ...
    if model_suffix:
        model_filename = (
            f"neural_network_chess_model_{num_train_samples}_{model_suffix}.pkl"
        )
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train neural network for chess evaluation"
    )
    ...
    for exp in experiments:
        ...
        train_neural_network(
            num_train_samples=100_000, plot_live=not args.no_plot, **exp
        )
