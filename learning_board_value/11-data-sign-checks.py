...
for example in ds:
    if example["cp"] is not None:
        positions.append({"fen": example["fen"], "cp": example["cp"]})
...
if piece_value_score <= 0:
    print(
        "   ❌ WARNING: piece_value_eval returned non-positive score for positive CP position!"
    )
else:
...
if piece_position_score <= 0:
    print(
        "   ❌ WARNING: piece_position_eval returned non-positive score for positive CP position!"
    )
else:
...
if piece_value_score >= 0:
    print(
        "   ❌ WARNING: piece_value_eval returned non-negative score for negative CP position!"
    )
else:
    print("   ✓ piece_value_eval sign is correct")
...
if piece_position_score >= 0:
    print(
        "   ❌ WARNING: piece_position_eval returned non-negative score for negative CP position!"
    )
else:
    print("   ✓ piece_position_eval sign is correct")
...
for label, board, cp_value in [
    ("Positive CP", board_positive, positive_pos["cp"]),
